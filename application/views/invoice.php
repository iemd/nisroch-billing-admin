...
<table style="width:100%;border: none;">
      ...
      <td style="border:none; padding: 0.25em;"><b>SGST :</b> Rs. <?php echo number_format((float)($row['gst']/2), 2, '.', ''); ?></td>
		  ...
		  <tr>
		    <?php
			   $dec_part = $row['payable_amount'] - floor($row['payable_amount']);
			   if($dec_part>0.5){
				$payable_amount = floor($row['payable_amount'])+1;
			   }else{
				$payable_amount = floor($row['payable_amount'])+0;
			   }
			?>
			<td style="border:none; padding: 0.25em;"><b>Net Value : Rs. </b> <?php echo $payable_amount; ?></td>
		  </tr>
		  </table>
